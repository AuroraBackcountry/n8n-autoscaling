...
- **Any n8n setting present in the environment becomes read-only in the UI.** Setting it in compose/`.env` makes the matching Settings field environment-managed, and a UI save that touches it fails with `Cannot update environment-managed fields: <field>` — rejecting the *entire* settings write, including unrelated fields saved alongside it. This is why the AI Assistant model and API key are deliberately not in compose. Put infrastructure in the environment; leave user-editable preferences and credentials to the UI.
